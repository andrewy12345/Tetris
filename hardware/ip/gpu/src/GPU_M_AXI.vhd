...
entity GPU_M_AXI is
	generic (
		...
		C_M_TARGET_SLAVE_BASE_ADDR	: std_logic_vector	:= x"00900000";
...
architecture implementation of GPU_M_AXI is
...
--	 							-- of the written data with the read data
    ...
    type state is (IDLE,BACKGROUND,TILE,SQUARE,DONE,ERASE,DRAW,REDRAW,ROW,COL);
    signal draw_state : state := BACKGROUND; 

	-- AXI4FULL signals
	--AXI4 internal temp signals
	signal awaddr	: unsigned(C_M_AXI_ADDR_WIDTH-1 downto 0);
	signal axi_awvalid	: std_logic;
	signal wdata	: unsigned(C_M_AXI_DATA_WIDTH-1 downto 0);
    ...
    signal x : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := (others => '0');
    
    signal xpos : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := (others => '0');
    signal ypos : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := (others => '0');
    
    signal xitr : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := (others => '0');
    signal yitr : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := (others => '0');
    signal zitr : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := (others => '0');
    
    signal old_status_reg : std_logic_vector(C_M_AXI_DATA_WIDTH-1 downto 0) := (others => '0');
    signal redraw_reg : std_logic_vector(C_M_AXI_DATA_WIDTH-1 downto 0) := (others => '0');
    
    constant hres : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := to_unsigned(1280, 32);
    constant vres : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := to_unsigned(1024, 32);
    
    constant startx : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := to_unsigned(380,32);
    constant starty : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := to_unsigned(23,32);
    constant increment : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0):= to_unsigned(55,32);
    
    constant white : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := X"00FFFFFF";
    constant black : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0) := X"00000000";
    constant grey : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0):= X"00888888";
    constant red : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0):= X"000000FF";
    constant blue : unsigned(C_M_AXI_DATA_WIDTH-1 downto 0):= X"00FF0000";
...
begin
	...
	M_AXI_AWADDR	<= std_logic_vector(unsigned(C_M_TARGET_SLAVE_BASE_ADDR) + awaddr);
    ...
    begin                                                           
        if(rising_edge(M_AXI_ACLK)) then                                 
            if(M_AXI_ARESETN = '0') then                                   
                awaddr <= (others => '0');
                wdata <= to_unsigned(sig_one, C_M_AXI_DATA_WIDTH);                                   
            else                                                             
                if(M_AXI_AWREADY = '1' and axi_awvalid = '1') then
                
                    case(draw_state) is
                        when IDLE => -- check for changes in input registers --
                           
                            if(old_status_reg /= status) then
                                old_status_reg <= status;
                                if(status(31) = '1') then
                                    draw_state <= REDRAW;
                                    redraw_reg <= erase_x1;
                                elsif(status(30) = '1') then
                                    draw_state <= ROW;
                                    redraw_reg <= erase_x1;
--                                elsif(status(29) = '1') then
--                                    draw_state <= COL;
--                                    redraw_reg <= erase_x1;
                                else
                                    draw_state <= ERASE;
                        ...
                        when BACKGROUND => -- draws a black background --
                        
                            if(awaddr <= (hres*vres*4)) then
                                wdata <= black;            
                                awaddr <= awaddr + unsigned(burst_size_bytes);
                            else
                                xitr <= (others => '0');
                                yitr <= (others => '0');
                                awaddr <= (others => '0'); 
                        ...
                        when TILE => -- draws a white foreground --
                            
                            if(yitr = 0 and xitr = 0) then
                                awaddr <= to_unsigned(320*4,32);
                                wdata <= white;
                                xitr <= xitr + 1;
                            else
                                if(xitr < 565) then
                                    ...
                                    xitr <= (others => '0');
                                    if(yitr <= vres) then
                                        awaddr <= to_unsigned(370*4,32) + resize(yitr*hres*4, 32);
                                        yitr <= yitr + 1;
                                    else
                                        draw_state <= SQUARE;
                                        xitr <= (others => '0');
                        ...
                        when SQUARE => -- draws squares on top of the foreground --
                        
                            if(xitr = 0 and yitr = 0) then
                                awaddr <= resize((startx + xpos*increment + (starty + ypos*increment)*hres)*4, 32);
                                wdata <= grey;
                                xpos <= xpos + 1;
                                xitr <= xitr + 1;
                            else
                        ...
                        when ERASE => -- used for updating the game screen, resets colored squares --
                        
                            if(yitr = 0 and xitr = 0) then
                                case(to_integer(zitr)) is
                                    when 0 =>
                                        if(unsigned(erase_x1) < 11 and unsigned(erase_y1) < 18) then
                                            xpos <= unsigned(erase_x1);
                                            ypos <= unsigned(erase_y1);
                                            awaddr <= resize((startx + unsigned(erase_x1)*increment + (starty + unsigned(erase_y1)*increment)*hres)*4, 32);
                                            wdata <= grey;
                                            xitr <= xitr + 1;
                                        end if;
                                        zitr <= zitr + 1;
                                    when 1 =>
                                        if(unsigned(erase_x2) < 11 and unsigned(erase_y2) < 18) then
                                            xpos <= unsigned(erase_x2);
                                            ypos <= unsigned(erase_y2);
                                            awaddr <= resize((startx + unsigned(erase_x2)*increment + (starty + unsigned(erase_y2)*increment)*hres)*4, 32);
                                            wdata <= grey;
                                            xitr <= xitr + 1;
                                        end if;
                                        zitr <= zitr + 1;
                                    when 2 =>
                                        if(unsigned(erase_x3) < 11 and unsigned(erase_y3) < 18) then
                                            xpos <= unsigned(erase_x3);
                                            ypos <= unsigned(erase_y3);
                                            awaddr <= resize((startx + unsigned(erase_x3)*increment + (starty + unsigned(erase_y3)*increment)*hres)*4, 32);
                                            wdata <= grey;
                                            xitr <= xitr + 1;
                                        end if;
                                        zitr <= zitr + 1;
                                    when 3 =>
                                        if(unsigned(erase_x4) < 11 and unsigned(erase_y4) < 18) then
                                            xpos <= unsigned(erase_x4);
                                            ypos <= unsigned(erase_y4);
                                            awaddr <= resize((startx + unsigned(erase_x4)*increment + (starty + unsigned(erase_y4)*increment)*hres)*4, 32);
                                            wdata <= grey;
                                            xitr <= xitr + 1;
                                        end if;
                                        zitr <= zitr + 1;
                        ...
                        when DRAW => -- color the squares at the input coordinates --
                        
                            if(yitr = 0 and xitr = 0) then
                                case(to_integer(zitr)) is
                                    when 0 =>
                                        if(unsigned(draw_x1) < 11 and unsigned(draw_y1) < 18) then
                                            xpos <= unsigned(draw_x1);
                                            ypos <= unsigned(draw_y1);
                                            awaddr <= resize((startx + unsigned(draw_x1)*increment + (starty + unsigned(draw_y1)*increment)*hres)*4, 32);
                                            wdata <= red;
                                            xitr <= xitr + 1;
                                        end if;
                                        zitr <= zitr + 1;
                                    when 1 =>
                                        if(unsigned(draw_x2) < 11 and unsigned(draw_y2) < 18) then
                                            xpos <= unsigned(draw_x2);
                                            ypos <= unsigned(draw_y2);
                                            awaddr <= resize((startx + unsigned(draw_x2)*increment + (starty + unsigned(draw_y2)*increment)*hres)*4, 32);
                                            wdata <= red;
                                            xitr <= xitr + 1;
                                        end if;
                                        zitr <= zitr + 1;
                                    when 2 =>
                                        if(unsigned(draw_x3) < 11 and unsigned(draw_y3) < 18) then
                                            xpos <= unsigned(draw_x3);
                                            ypos <= unsigned(draw_y3);
                                            awaddr <= resize((startx + unsigned(draw_x3)*increment + (starty + unsigned(draw_y3)*increment)*hres)*4, 32);
                                            wdata <= red;
                                            xitr <= xitr + 1;
                                        end if;
                                        zitr <= zitr + 1;
                                    when 3 =>
                                        if(unsigned(draw_x4) < 11 and unsigned(draw_y4) < 18) then
                                            xpos <= unsigned(draw_x4);
                                            ypos <= unsigned(draw_y4);
                                            awaddr <= resize((startx + unsigned(draw_x4)*increment + (starty + unsigned(draw_y4)*increment)*hres)*4, 32);
                                            wdata <= red;
                                            xitr <= xitr + 1;
                                        end if;
                                        zitr <= zitr + 1;
                        ...
                        when REDRAW => -- redraws the whole block/tile area to reflect the updated screen
                            
                            if(xitr = 0 and yitr = 0) then
                                awaddr <= resize((startx + xpos*increment + (starty + ypos*increment)*hres)*4, 32);
                                if(redraw_reg(to_integer(xpos)) = '1') then
                                    wdata <= red;
                                else
                                    wdata <= grey;
                                end if;
                                            ...
                                            if(ypos < 17) then
                                                ...
                                                case(to_integer(ypos)) is
                                                    ...
                                                    when 14 =>
                                                        redraw_reg <= draw_y4;
                                                    when 15 =>
                                                        redraw_reg <= extra_reg0;
                                                    when 16 =>
                                                        redraw_reg <= extra_reg1;
                                                     when others =>
                                                end case;
                                            else
                                                draw_state <= DONE;
                        ...
                        when ROW => -- highlights the rows as an effect --
                        
                            if(xitr = 0 and yitr = 0) then
                                if(redraw_reg = X"000003FF") then
                                    awaddr <= resize((startx + xpos*increment + (starty + ypos*increment)*hres)*4, 32);
                                    wdata <= unsigned(extra_reg2);
                                    xpos <= xpos + 1;
                                    xitr <= xitr + 1;
                                else
                                    ypos <= ypos + 1;
